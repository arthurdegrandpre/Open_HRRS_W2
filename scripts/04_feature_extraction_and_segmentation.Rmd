...
This fourth script is used to extract additionnal textural and statistical information from the images, using local statistical moments, haralick textures and edge extraction from OrfeoToolBox in order to perform an adequate segmentation of the image prior to glint correction. OrfeoToolBox (OTB) is an open-source high resolution image analysis software code in C++ that can run within it's own GUI, or can be called from command line, allowing its use within R or Python. The feature extraction will be applied onto bottom of atmosphere mosaic images as produced in the previous scripts, 03_atmospheric_correction.
...
# Select images for glint correction
...
(CEGRIM SPECIFIC SECTION)

```{r years_to_fix}
years = c(2011,2013,2017,2019)
...
feature.LocalStatisticExtraction(
                           ...
                           name      = paste0("ls_",str_sub(glint_imgs[i],-46)),
...
feature.EdgeExtraction(
                           ...
                           name      = paste0("ee_",str_sub(glint_imgs[i],-46)),
                           channel = "8",   #default 1, selected channel index in input image
...
feature.HaralickTextureExtraction(
                           ...
                           name      = paste0("ht_",str_sub(glint_imgs[i],-46)),
                           channel = "8",   #default 1, selected channel index in input image
                           texture   = "simple",  #default simple, also available "advanced" or "higher"
                           parameters.min = "0", #input image minimum value
                           parameters.max = "1", #input image maximum value
...
for(i in seq_along(glint_imgs)){
  ...
  writeRaster(r, paste0(output_dir,str_sub(glint_imgs[i],-46)), overwrite=T)
}
```
