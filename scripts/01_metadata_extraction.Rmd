...
All libraries must be installed prior to loading them. Installing them individually is simple using the install.packages("nameofthepackage") function, which can also be used to update the packages.
...
```{r finding images}
imgs_dir = "C:/Users/Arthur/Documents/data/mffp" # directory containing all satellite images (can be in different folders, to be found with recursive search)
```
...
imgs = as.data.frame(
  dir(imgs_dir, recursive = T, pattern = ".TIF$")
