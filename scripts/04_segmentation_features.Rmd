...
For a better classification, additional features can be added to the image, such as texture, local statistics and edge extractions. All those layers can be used as additional when classifying objects.
...
for(i in seq_along(MUL_input)){
  ...
  meanshift.segm(filter.meanshift.spatialr = "5",      # default 5
               ...
               name      = str_split(basename(MUL_input[i])))
...
feature.HaralickTextureExtraction(
                           ...
                           name      = paste0("n_ht_",basename(MUL_input[i])),
...
for(i in seq_along(MUL_input)){
 ...
 r = brick(MUL_input[i])
  ...
  writeRaster(r, paste0(features_output,basename(MUL_input[i])), overwrite=T)
}
